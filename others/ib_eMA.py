...
def MACD(DF,a=9,b=27,c=54):

    df = DF.copy()
    df["MA_9"]=df["Close"].ewm(span=a,min_periods=a).mean()
    df["MA_27"]=df["Close"].ewm(span=b,min_periods=b).mean()
    df["MA_54"] = df["Close"].ewm(span=c, min_periods=c).mean()
...
macdDF = {}
for ticker in tickers:
    macdDF[ticker] = MACD(historicalData[ticker],9,27,54)

print(macdDF)
macdDF['SPY'].to_csv('eMA.csv')